      ...
      body: ListView.builder(
          ...
          itemBuilder: (BuildContext context, int index) {
            return TagContainer(
                searchTag: tag[index],
                onTap: (String title) {
                  context.watch<SearchState>().selectedTagHandle(title);
                });
          }),
    );
  }
...
class TagContainer extends StatelessWidget {
  ...
  const TagContainer({Key? key, required this.searchTag, required this.onTap})
      : super(key: key);

  @override
  Widget build(BuildContext context) {
    List<Widget> tagWidgetList = [];
    for (String title in searchTag.data) {
      tagWidgetList.add(
        tagDetail(title, false),
      );
      // tagWidgetList.add(
      //   Container(
      //     padding: EdgeInsets.all(Adapt.px(10)),
      //     decoration: BoxDecoration(
      //         color: context.watch<SearchState>().selectedTag.indexOf(item) > -1
      //             ? Colors.orange
      //             : Colors.black,
...
Widget tagDetail(item, selected) {
  return InkWell(
    onTap: () {},
    child: Container(
      padding: EdgeInsets.all(Adapt.px(10)),
